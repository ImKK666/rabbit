...
> **补充（2026-08-19，R-49）**：这一轮做的是**审美**，而结论仍然落在第九节那句话上 ——
> 「排得整齐但没有设计感」的根因不是配色不好看，是**版式引擎把「一页有多高」当成已知常量在用**
> （固定 `top:150`、固定 `height:52`），内容多了压在一起、少了留一大片空。
> 处置是把「按内容构图」写进代码：`stack()` 垂直编排 + 8px 基线栅格 + 按内容降级。
>
> 第四节那张表这一轮又被兑现了一次，而且是最刺眼的一次：**66 张样张跑完整 lint 得到 0 条告警，
> 其中好几张肉眼可见文字压在一起**。因为 `Builder.text()` 会把框高夹进画布，
> 于是「超出画布」这条检查**结构上就不可能响**。判据立起来的办法是换个量法：
> 不问「框在哪」，问「字画到哪了」——`npm run layout-text` 在真浏览器里量，
> 15 处溢出全部清零。工具见 `npm run layout-shoot` / `layout-audit` / `layout-text`。
>
> 遮罩浓度那两个常量（0.82 / 0.78）实测是**把照片压没了**，现在按图片实际亮度算
> （`imageCodec` 解码时顺手量 p5/p95）。版式 10 → 13，配色加了四个风格包。
> 详见 [04-changes.md 第二十轮](./04-changes.md#2026-08-19-第二十轮审美--把按内容构图写进代码r-49)。
...
- ~~**`configs/shapes.ts` 里 51 个图标字形**没起名字 —— 1024 viewBox 的复杂路径，光看 path 没法可靠命名，猜错比没有更糟~~
  ...
  > **R-41 已做（2026-08-19）**：前半句对，结论下早了 —— 看不出来是因为**没把它们画出来看**。
  > `npm run shapes` 生成联系表、`shoot-shape-sheet.mjs` 截图，逐个看着命名，
  > 存疑的放大再看。51 个收了 47（排除 3 个第三方品牌标识 + 1 个孤零零的 ♂，理由钉在测试里），
  > 目录 37 → 86。详见 [04-changes.md 第十二轮](./04-changes.md#2026-08-19-第十二轮图标字形命名r-41)。
  >
  > 这条留在这里当个记号：**「没法可靠判断」有时候只是「还没找到看它的方式」。**
  > 和 R-36 用逐帧采样代替肉眼、R-39 用联系表代替读代码是同一件事。
