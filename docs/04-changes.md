...
用户报「生成的 PPT 风格趋近统一」。先查了库里 7 份真实 deck 当实证：
**4 份的主题是出厂默认（白底 + `#5b9bd5` 蓝 + `#ed7d31` 橙），动都没动**；
「星耀影视」的 designNote 写了一长串点茶取色、实际颜色却还是默认蓝橙 ——
**写了说明，没做决定**。cards 在每份稿子里都是绝对主力。趋同是真的。

五件事一起做（A~E），全部带判据：

### A · 堵 lint ⑨ 的洞

旧判据只看 `designNote` 非空，「星耀影视」就是反例。现在必须**说了而且做了**：
designNote 非空 **且** 主题锚点色偏离内置默认主题（`DEFAULT_THEME` 收成
`kernel.ts` 的代码常量，`pipeline.ts` 共用 —— 旧注释担心的「默认主题一改判据失准」
不存在了）。只写 note 不改色 → 报「写说明不等于做了决定」。
`setTheme` 工具入口同步堵：designNote 必须伴随至少一个颜色键。
`sameHex` 走 `parseHex` 比较 —— 手写的「去 # + 小写」把 `#fff` ≡ `#FFFFFF`
判成了不同，是测试当场抓的。

### B · artDirection：生成图层的艺术方向由稿子决定

原来 `backdrop.ts` 的 Style 行**写死** `flat vector / editorial print` ——
每份稿子的底图都是同一张脸，是趋同的最大一档来源（一旦默认开底图就会兑现）。
现在：

- `theme.artDirection`（英文短语，模型在 setTheme 里写）注入
  `buildBackdropPrompt` / `buildOrnamentPrompt`；没写就按页面的质感档位
  回落 `design.ts` 的 `ART_DIRECTIONS`（每档一张不同的脸）。
- **结构约束一律不动**：安静区 / 禁文字 / (A)(B) 两条装饰约束照旧 ——
  阈值是拿真样本标定的，默认路径一个字都不漂。测试钉死这一点。
- prompt 教模型写具体流派（"mid-century editorial illustration"），
  不写 professional / clean 这种空词。

### C · signature 每档质感两个变体

原来每档风格只有**一个**版面记号 —— 两份都用 tech 的稿子记号长得一模一样。
现在 `SIGNATURES[style][0|1]`（8 档变体 + editorial / soft 新档各 2 = 12 种），
由主题锚点色的哈希稳定选一个：同稿稳定、跨稿分散。**选变体是代码的事，
模型一个参数都不碰。** 全部变体只住左/上页边距 —— 吃图版式里配图贴右出血，
右下角的记号会压在照片上，`layoutImage.test.ts` 当场抓住了第一版。

### D · 三个高频版式的结构变体 B

`applyLayout` 加 `variant: 'A' | 'B'`（落盘 `slide.layoutVariant`）：

| 版式 | B 变体 |
|---|---|
| title-center | 左对齐封面（右侧大色块装饰，刊物感） |
| bullets | 大编号列表（30px 大号数字，去圆点） |
| cards | 分栏无卡（去卡片底板，栏间竖线，杂志分栏） |

lint ① 的键从「版式名」扩成「版式名 + 变体」—— 同版式不同变体是两种结构，
相邻不报；同版式同变体报，且提示里有变体出路。顺手修掉 ① 里写死的
「applyLayout 有 10 种」（13 种了也没人发现），改成 `LAYOUT_PATTERNS.length`。

### E · 菜单扩容：+2 质感档位 +2 字体 +1 配对

- 质感档位 4 → 6：**editorial**（编辑风：描边最重、墨感、强调色压墨）与
  **soft**（柔和：描边最轻、暖染最高）。表单是数据不是 enum ——
  `tools.ts` 的两个 `z.enum([...])` 改为从 `PALETTE_STYLES` 派生，
  加风格不会再有「抄了名单的 enum 悄悄落后」。
- 字体 8 → 10：**朱雀仿宋** + **文鼎PL楷体**，`npm run char-width` 真浏览器实测
  （朱雀拉丁 0.447 偏窄、文鼎楷拉丁是 0.5em 半角等宽 —— 都写进 FONT_NOTES）。
- 配对 6 → 7：**heritage**（朱雀仿宋标题 + 思源黑正文，文化/古籍题材）。
- prompt 里写死的「从这八个里挑 / 六套现成」全部改成中性措辞，清单自动跟着长。

### 判据

新增 20 条测试（lint ⑨ 同色异写 / artDirection 注入与默认路径 / 12 个 signature
变体 / 3 个 B 变体的几何与动画覆盖 / 新档位配方两端性 / 新字体表）。
全量 **1718 测试绿**，`bunx tsc --noEmit` 与 `vue-tsc --build` 都 exit 0。
顺手修掉了后端 tsc 在 HEAD 上就存在的一处测试类型收窄错误
（`'left' in el` 收窄不掉 PPTLineElement）。

### 没做的（边界）

- **没有真跑一次完整生成**（和 R-55/R-58 欠的同一笔）：五件套的效果
  —— 尤其 B 的 artDirection 注入后生图模型的表现 —— 只有端到端实测才知道。
- F（跨稿记忆：生成时把这个用户最近 N 份稿子的 designNote 注进 prompt 当反例）
  与 G（设计阶段单独调采样温度）本轮没做。
- max-lines 那条 lint 报错在 HEAD 上就存在（kernel 1765 / layouts 1893 行），
  本轮没有为它拆文件。
...
**OpenAI 接口（image2）没有每分钟限流** —— 本地限流器的有效限额按形状落定：
openai → `null`（一律放行），gemini → 库里配的值（那个中转实测 2~3 次/分钟就 429）。
`effectiveImageRateLimit` 纯函数 + 判据两条；三处调用点（装饰层/底图/内容配图）同步。
设置页与 prompt 的「每分钟配额」措辞改为只对 Gemini 形状生效。
...
| ~~**生图 prompt 模板化**~~ | ~~现在 agent 写什么就发什么，产出风格一致性全靠模型自觉。~~ **R-60 已落实**：艺术流派由模型在 setTheme 里写一次（`artDirection`），结构 / 安静区 / 配色全部由代码注入，没写时按质感档位回落 `ART_DIRECTIONS` | ✅ |
